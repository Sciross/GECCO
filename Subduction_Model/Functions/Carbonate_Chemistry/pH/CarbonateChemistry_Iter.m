function [pH,CO2,HCO3,CO3,Omega,Ab] = CarbonateChemistry_Iter(Constant,DIC,AlkalinityT,HIn,CarbonateConstants);
...
% Should I be including fluoride??
...
while Anomaly>Tol;
    ...
    Ah3po4g = (Constant.Carbonate_Phosphate.*HIn.^3)./denom;
%     Ah2po4g = (Constant.SurfacePhosphate*kp1*HIn.^2)/denom;
    Ahpo4g = (Constant.Carbonate_Phosphate.*kp1.*kp2.*HIn)./denom;
    Apo4g = (Constant.Carbonate_Phosphate.*kp1.*kp2.*kp3)./denom;
    
    % Borate
    Ab = (kb.*Constant.Carbonate_Boron)./(kb+HIn);
    
    % OH
    Aoh = (kw./HIn);
    
    % Silica
    Asi = (ksi.*Constant.Carbonate_Silica)./(ksi+HIn);
    
    % Correction
    Fh = nansum([HIn,+Ah3po4g,-Ab,-Aoh,-Asi,-Ahpo4g,-(2.*Apo4g)],2);
...
%     Ca = 10.28; %mol/m^3 ## SHOULD SPECIFY OUTSIDE OF FUNCTION
    ...
    Omega = (Constant.Carbonate_Calcium.*CO3)./(Ksp);
