function Coefficients = Alteration(Coefficients,Number,Value);
    if numel(Number)==1;
        x = [0;15]+273.15;
        y = (Coefficients(1)*exp(Coefficients(2)*x)+Coefficients(3));
        ...
        if Number == 1;
            ...
            Disparity = Weathering_One(x,y,Value,b_Current);
            ...
            b_Current = b_Query(2);
            Disparity(2) = Weathering_One(x,y,Value,b_Current);
            ...
            while abs(Disparity_Current)>Tolerance;
                ...
                Disparity_Current = Weathering_One(x,y,Value,b_Current);
                % Replace the b value
            ...
            Coefficients(3) = y(2)-(Coefficients(1).*exp(Coefficients(2).*x(2)));
        end
    elseif numel(Number)==3;
        ...
        if isnan(Value(1));
            Value(1) = (y-Value(3))./(exp(Value(2).*x));
        elseif isnan(Value(2));
            Value(2) = (1./x).*(log((y-Value(3))/Value(1)));
        elseif isnan(Value(3));
            Value(3) = y-(Value(1).*exp(Value(2).*x));
        end
