...
function [yt_Out,V_out,L_out,S_out,ST_out,P_out] = MySolver_2_Implicit_Trial(ODE,Model,Time_In,Time_Out,y0,y0_Sub,y0_Meta);
    % Where Times are vectors    
    Step_Size_In = Model.dt;
    Step_Size_Out = Time_Out(2)-Time_Out(1);
    ...
    if isnan(Model.Conditions.Present.Core_Depths);
        S_out = NaN;
    else
        S_out = NaN(numel(Model.Conditions.Present.Core_Depths),numel(Time_Out));
    end
    P_out = NaN(2,numel(Time_Out));
    
    % Run once to update present conditions ### WHY?
    ODE(0,y0,y0_Sub,y0_Meta);
    
    if ~Var_EmptyFlag;
        for ParameterNumber = 1:size(Model.Conditions.Variable,1);
            if ~isnan(Model.Conditions.Variable{ParameterNumber,2});
                V0(ParameterNumber,1) = Model.Conditions.Present.(Model.Conditions.Variable{ParameterNumber,1}{1})(Model.Conditions.Variable{ParameterNumber,2});
            else
                V0(ParameterNumber,1) = NaN;
            end
        end
        V = V0;
    else
    ...
    if numel(Time_In)==numel(Time_Out);
        if sum(Time_In==Time_Out)==numel(Time_Out);
            ...
            if Time_Out(1)==0 && Time_In(1)==0;
                ...
                L_out(:,1) = L0;
                if ~isnan(Model.Conditions.Present.Core_Depths);
                    S_out(:,1) = y0_Sub(Model.Conditions.Present.Core_Depths);
                end
                ST_out(:,1) = sum(y0_Sub);
...
%     end
    ...
    elseif rem(Step_Size_Out,Step_Size_In)==0 && Time_Out(1)==Time_In(1);
        ...
        if Time_Out(1)==Time_In(1);
            ...
            L_out(1) = L0;
            if ~isnan(Model.Conditions.Present.Core_Depths);
                S_out(:,1) = y0_Sub(1001-(Model.Conditions.Present.Core_Depths/10));
            end
            ST_out(:,1) = sum(y0_Sub);
            P_out(:,1) = Model.Conditions.Present.PIC_Burial;
            Place = 2;
        end
        ...
        try
            for Current_Step = 2:numel(Time_In);
                ...
                [dy_1,dy_1_Sub,dy_1_Meta] = ODE(Time_In(Current_Step-1),y_1,y_1_Sub,y_1_Meta);
...
%                 ODE(Time_In(Current_Step),y_2,y_2_Sub,y_2_Meta);
                ...
                if Count==Count_Reached;
                    yt_Out(:,Place) = y_2;
                    % Lysocline
                    L_out(:,Place) = Model.Conditions.Present.Lysocline;
                    if ~isnan(Model.Conditions.Present.Core_Depths);
                        S_out(:,Place) = y_2_Sub(1001-(Model.Conditions.Present.Core_Depths/10));
                    end
                    ST_out(1,Place) = sum(y_2_Sub);
                    P_out(:,Place) = Model.Conditions.Present.PIC_Burial;
                    
                    if ~Var_EmptyFlag;
                        for ParameterNumber = 1:size(Model.Conditions.Variable,1);
                            if isnan(Model.Conditions.Variable{ParameterNumber,2});
                                V(ParameterNumber,1) = NaN;
                            else
                                V(ParameterNumber,1) = Model.Conditions.Present.(Model.Conditions.Variable{ParameterNumber,1}{1})(Model.Conditions.Variable{ParameterNumber,2});
                            end
                        end
                        V_out(:,Place) = V;
                    end
            ...
            Model.Outgassing = y_2_Meta;
            Model.Seafloor = y_2_Sub;
            return;
        end
...
%                 s1 = subplot(7,1,1);
%                 hold on
%                 plot(Time_In(Current_Step),y_1(1).*Model.Conditions.Present.Atmosphere_Volume,'.k');
%                 
%                 s2 = subplot(7,1,2);
%                 hold on
%                 plot(Time_In(Current_Step),y_1(5).*Model.Architectures.BoxDepths(1).*Model.Architectures.BoxArea,'.k');
%                 
%                 s3 = subplot(7,1,3);
%                 hold on
%                 plot(Time_In(Current_Step),y_1(6).*Model.Architectures.BoxDepths(2).*Model.Architectures.BoxArea,'.k');
%                 
%                 s4 = subplot(7,1,4);
%                 hold on
...
%                 
%                 if rem(Time_In(Current_Step),Model.Conditions.Present.Metamorphism_Resolution)==0;
%                     RemainingInBox = 0;
%                 else
%                     RemainingInBox = (1-rem(Time_In(Current_Step),Model.Conditions.Present.Metamorphism_Resolution)/Model.Conditions.Present.Metamorphism_Resolution)*(y_1_Meta(1+floor(Time_In(Current_Step)/Model.Conditions.Present.Metamorphism_Resolution)));
%                 end
%                 Total = (y_1(1).*Model.Conditions.Present.Atmosphere_Volume) + (y_1(5).*Model.Architectures.BoxDepths(1).*Model.Architectures.BoxArea) + (y_1(6).*Model.Architectures.BoxDepths(2).*Model.Architectures.BoxArea) + y_1(13) + sum(y_1_Sub) + sum(y_1_Meta((2+floor(Time_In(Current_Step-1)/Model.Conditions.Present.Metamorphism_Resolution)):end)) + RemainingInBox;
%                 
%                 subplot(7,1,7);
%                 hold on
...
%             Time_In = Time_In(2:end);
        ...
        for Current_Step = 1:numel(Time_In);
            ...
            if Current_Step==Interpolation_At(Count);
               ...
               if ~Var_EmptyFlag;
                   for ParameterNumber = 1:size(Model.Conditions.Variable,1);
                       V(1,ParameterNumber) = Model.Conditions.Present.(Model.Conditions.Variable{ParameterNumber,1}{1})(Model.Conditions.Variable{ParameterNumber,2});
                   end
