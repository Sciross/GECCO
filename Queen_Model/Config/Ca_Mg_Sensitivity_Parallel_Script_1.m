    ...
    for Time_Index = 1:numel(Change_Times);
        for Calcium_Change_Index = 1:numel(Calcium_At_Change);
            ...
            for Silicate_Change_Index = 1:numel(Coefficients);
                ...
                Gecco.Runs(Silicate_Change_Index).Information.Output_Filepath = "/scratch/rw12g11/Ca_Mg_Silicate_Ensemble/10e6";
                Gecco.Runs(Silicate_Change_Index).Information.Output_Filename = strcat("T",num2str(Change_Times(Time_Index)),"CC",num2str(Calcium_At_Change(Calcium_Change_Index)),"MC",num2str(Magnesium_At_Change(Magnesium_Change_Index)),"SW",num2str(Silicate_Change_Index),".nc");
                ...
                Calcium_m(2) = (Calcium_End-Calcium_Point)/(Gecco.Runs(Silicate_Change_Index).Chunks(2).Time_In(2)-Calcium_Time);
                ...
                Magnesium_m(2) = (Magnesium_End-Magnesium_Point)/(Gecco.Runs(Silicate_Change_Index).Chunks(2).Time_In(2)-Magnesium_Time);
                
                Magnesium_c(1) = Magnesium_Start;
                Magnesium_c(2) = Magnesium_Point-(Magnesium_m(2)*Magnesium_Time);
                
                % Add the right transients
                Gecco.Runs(Silicate_Change_Index).Regions.Conditions.Transients.Carbonate_Chemistry.Calcium(1,1:3) = {1,':',str2func(strcat("@(t,Conditions)(",num2str(Calcium_m(1)),".*t)+",num2str(Calcium_c(1))))};
                Gecco.Runs(Silicate_Change_Index).Regions.Conditions.Transients.Carbonate_Chemistry.Calcium(2,:) = {2,':',str2func(strcat("@(t,Conditions)(",num2str(Calcium_m(2)),".*t)+",num2str(Calcium_c(2))))};
                
                Gecco.Runs(Silicate_Change_Index).Regions.Conditions.Transients.Carbonate_Chemistry.Magnesium(1,1:3) = {1,':',str2func(strcat("@(t,Conditions)(",num2str(Magnesium_m(1)),".*t)+",num2str(Magnesium_c(1))))};
                Gecco.Runs(Silicate_Change_Index).Regions.Conditions.Transients.Carbonate_Chemistry.Magnesium(2,:) = {2,':',str2func(strcat("@(t,Conditions)(",num2str(Magnesium_m(2)),".*t)+",num2str(Magnesium_c(2))))};
