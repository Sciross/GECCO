...
%                 self.Data_Names = {'Atmosphere_CO2','Algae','Phosphate','DIC','Alkalinity'};
        ...
        %% Saving Data
        function SaveData(self,Filename,Run_Index,Region_Index);
        ...
        function SaveConstants(self,Filename,Run_Index,Region_Index);
            % Open file
            FileID = netcdf.open(Filename,'WRITE');
        ...
        function SaveTransients(self,Filename,Run_Index,Region_Index);
            % Open file
            FileID = netcdf.open(Filename,'WRITE');
        ...
        function SaveAttributes(self,Filename);
            % Open file
            FileID = netcdf.open(Filename,'WRITE');
...
%             netcdf.putAtt(FileID,GlobalID,'Model',self.Model);
            netcdf.putAtt(FileID,GlobalID,'Version_Number',self.Information.Version_Number);
            netcdf.putAtt(FileID,GlobalID,'Version_Codename',char(self.Information.Version_Codename));
...
%             netcdf.putAtt(FileID,GlobalID,'Solver',self.Solver);
        ...
        function Save(self,Filename,Run_Index,Region_Index);
            self.SaveAttributes(Filename);
            self.SaveData(Filename,Run_Index,Region_Index);
            self.SaveConstants(Filename,Run_Index,Region_Index);
%             self.SaveTransients(Filename,Run_Index,Region_Index);
        ...
        function Replication_Data = MakeReplicationData(self,Run_Data_In);
            ...
            Run_Data = Run_Matrix;
%             if ~isempty(self.Conditions.Perturbations.Perturbation_Matrix);
%                 Perturbation_Data = char(join(self.Conditions.Perturbations,','));
%             else
%                 Perturbation_Data = [];
%             end
%             if size(self.Conditions.Transients,1)>0 && isempty(self.Conditions.Transients{2});
%                 Variable_Data = char(join(self.Conditions.Transients([1,3:end]),','));
%             elseif ~isempty(self.Conditions.Transients);
%                 Variable_Data = char(join(self.Conditions.Transients,','));
%             else
%                 Variable_Data = [];
%             end
