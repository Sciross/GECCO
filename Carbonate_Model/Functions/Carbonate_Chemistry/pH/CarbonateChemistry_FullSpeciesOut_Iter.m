...
while Anomaly>Tol;
    ...
    % Other components
    B3 = Constant.Boron./(1 + kb./HOut);
    B4 = Constant.Boron./(1 + HOut./kb);
    
    HPlus = HOut;
    OH = kw./HPlus;
    
    H3PO4 = Constant.Phosphate./(1 + kp1./HOut + (kp1.*kp2)./(HOut.^2) + (kp1.*kp2.*kp3)./(HOut.^3));
    H2PO4 = Constant.Phosphate./(HOut./kp1 + 1 + kp2./HOut + (kp2.*kp3)./(HOut.^2));
    HPO4 = Constant.Phosphate./((HOut.^2)./(kp1.*kp2) + HOut./kp2 + 1 + kp3./HOut);
    PO4 = Constant.Phosphate./((HOut.^3)./(kp1.*kp2.*kp3) + (HOut.^2)./(kp2.*kp3) + HOut./kp3 + 1);
    
    H3SiO4 = Constant.Silica./(HOut./ksi + 1);
    H4SiO4 = Constant.Silica./(1 + ksi./HOut);
    
    F = Constant.Fluoride./(HOut./kf + 1);
    HF = Constant.Fluoride./(1 + kf./HOut);
