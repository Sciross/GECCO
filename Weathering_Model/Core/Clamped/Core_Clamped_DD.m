...
% DIC
DICRiverineFlux = y(17)*GECCOObject.Conditions.Present.Riverine_Volume;
...
%% Alkalinity Fluxes
AlkalinityRiverineFlux = y(18)*GECCOObject.Conditions.Present.Riverine_Volume;
...
Carbonate.Buried = (AddArray./(sum(AddArray))).*(PICBurialFlux+POCBurialFlux);
...
% if t~=0 && rem(GECCOObject.Current_Step,2)==0;
%     Outgassing = ((y_Meta(1+floor((t-20)/GECCOObject.Conditions.Present.Metamorphism_Resolution))/GECCOObject.Conditions.Present.Metamorphism_Resolution));
% elseif t~=0 && rem(GECCOObject.Current_Step,2)~=0;
...
%% Weathering
Carbonate_Weathering = (y(13).*y(15));
Silicate_Weathering = (y(12).*y(14));

Weathering = 2*(Silicate_Weathering + Carbonate_Weathering);
...
%% Assign dys
% Flux over volume
dy(1) = CO2.Flux./GECCOObject.Conditions.Present.Atmosphere_Volume;
...
%     s1 = subplot(7,1,1);
%     hold on
%     plot(t,y(1).*GECCOObject.Conditions.Present.Atmosphere_Volume,'.k');
%     
%     s2 = subplot(7,1,2);
%     hold on
%     plot(t,y(5).*GECCOObject.Architectures.BoxDepths(1).*GECCOObject.Architectures.BoxArea,'.k');
%     
%     s3 = subplot(7,1,3);
%     hold on
%     plot(t,y(6).*GECCOObject.Architectures.BoxDepths(2).*GECCOObject.Architectures.BoxArea,'.k');
%     
%     s4 = subplot(7,1,4);
%     hold on
%     plot(t,y(13),'.k');
...
%     plot(t,sum(y_Meta),'.k');
%     
%     Total = (y(1).*GECCOObject.Conditions.Present.Atmosphere_Volume) + (y(5).*GECCOObject.Architectures.BoxDepths(1).*GECCOObject.Architectures.BoxArea) + (y(6).*GECCOObject.Architectures.BoxDepths(2).*GECCOObject.Architectures.BoxArea) + y(13) + sum(y_Sub) + sum(y_Meta((1+floor(t/GECCOObject.Conditions.Present.Metamorphism_Resolution)):end)) - (rem(t,GECCOObject.Conditions.Present.Metamorphism_Resolution)/GECCOObject.Conditions.Present.Metamorphism_Resolution)*(y_Meta(1+floor(t/GECCOObject.Conditions.Present.Metamorphism_Resolution)));
%     
%     subplot(7,1,7);
%     hold on
%     plot(t,Total,'.k');
%     plot(t,Total-(Outgassing*20),'or');
%     
%     set([s1,s2,s3,s4,s5,s6],'XTick',[]);
%     drawnow;
% end
% GECCOObject.Current_Step = GECCOObject.Current_Step + 1;
end
