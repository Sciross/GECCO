    ...
    methods
            ...
            if strcmp(event.NewValue.Title,'Plot Data');
                if ~isempty(self.Model);
                    if ~isempty(self.Model.Data);
                        ...
                        subplot(self.s{1});
                        p{1} = plot(self.Model.Time(1,:,1),self.Model.Data(1,:,1)*1000000);
                        ylabel({'CO_2','(ppm)'});
        ...
        function SelectPlot(self,src,event);
            ...
            if self.SubplotIndex==1;
                ...
                subplot(self.s{3});
                p{3} = plot(self.Model.Time(1,:,1),self.Model.Data(3:4,:,self.PlotRunSelectorUI.Value));
                ylabel({'Phosphate','(mol/m^3)'});
                
                subplot(self.s{4});
                p{4} = plot(self.Model.Time(1,:,1),self.Model.Data(5:6,:,self.PlotRunSelectorUI.Value));
                ylabel({'DIC','(mol/m^3)'});
                
                subplot(self.s{5});
                p{5} = plot(self.Model.Time(1,:,1),self.Model.Data(7:8,:,self.PlotRunSelectorUI.Value));
                ylabel({'Alkalinity','(mol/m^3)'});
            ...
            elseif self.SubplotIndex==2;
                subplot(self.s{1});
                p{1} = plot(self.Model.Time(1,:,self.PlotRunSelectorUI.Value),self.Model.Data(9:11,:,self.PlotRunSelectorUI.Value)-273.15);
                ylabel({'Temperature','(^{\circ}C)'});
                
                subplot(self.s{2});
                p{2} = plot(self.Model.Time(1,:,self.PlotRunSelectorUI.Value),self.Model.Data(12:13,:,self.PlotRunSelectorUI.Value));
                ylabel({'Rock','(mol)'});
                ...
                p{3} = plot(self.Model.Time(1,:,self.PlotRunSelectorUI.Value),Silicate_Weathering_Fraction);
                hold on
                ...
                subplot(self.s{4});
                p{4} = plot(self.Model.Time(1,:,1),self.Model.Data(16,:,self.PlotRunSelectorUI.Value));
            ...
            elseif self.SubplotIndex==3;
                subplot(self.s{1});
                p{1} = plot(self.Model.Time(1,:,self.PlotRunSelectorUI.Value),self.Model.Lysocline(:,:,self.PlotRunSelectorUI.Value));
                ylabel({'Lysocline','(m)'});
                ...
                % Calculate true outgassing
%                 Outgassing_Amp = (self.Model.Outgassing/self.Model.Conditions.Present.Outgassing_Temporal_Resolution)./(self.Model.Conditions.Initial.Outgassing(1)/self.Model.Conditions.Present.Outgassing_Temporal_Resolution);
%                 Carbonate_Amp = self.Model.Data(13,:)./self.Model.Conditions.Initial.Carbonate;
%                 Outgassing_Amp2 = (Outgassing_Amp(1:30000));
                
                subplot(self.s{3});
                p{3} = plot(self.Model.Conditions(self.PlotRunSelectorUI.Value).Constant.Outgassing_Temporal_Resolution*(1:numel(self.Model.Outgassing(:,:,self.PlotRunSelectorUI.Value))),self.Model.Outgassing(:,:,self.PlotRunSelectorUI.Value));
                ylabel({'Outgassing','(mol)'}); 
%                 hold on
%                 plot((1:300001)*1000,[repelem(Outgassing_Amp2,10);NaN]'.*Carbonate_Amp.*self.Model.Outgassing(1:30001)');
            end
        end
        ...
        function RecreateSubplots(self,src,event);
            if self.SubplotIndex == 1 || self.SubplotIndex==2;
                ...
                self.s = cell(1,3);
                for n = 1:3;
                    self.s{n} = subplot(3,1,n,'Parent',self.Tab2Handle);
                end
            end
        end
        %% Filepath Callbacks
...
%                 Names = fieldnames(self.Model.Conditions(1).Constant);
        ...
        function InitTableEditCallback(self,src,event);
            ...
            self.Model.Conditions.Initial(event.Indices(2)).Conditions = [self.Model.Conditions.Initial(event.Indices(2)).Atmosphere_CO2;
                                                                          ...
                                                                          self.Model.Conditions.Initial(event.Indicies(2)).Ice];
        end
...
%             Table = horzcat(self.Model.Conditions.Constant(1:end).(src.String{src.Value}));
        ...
        function Validate(self,src,event);
            ...
            elseif sum(Max_Outgassing_Tracked==Initial_Outgassing_Length | Initial_Outgassing_Length==0 | Initial_Outgassing_Length==1 | Initial_Outgassing_Length==2)==0;
                self.UpdateLogBox(['Initial Outgassing array is the wrong length, should be...',num2str(Max_Outgassing_Tracked),' elements']);
            else
        ...
        function CalculateDependents(self,src,event);
            ...
            for Run_Index = 1:numel(self.Runs);
                if self.Model.Conditions(Run_Index).Constant.Outgassing_Spread==0;
                    ...
                    self.Model.Conditions(Run_Index).Present.Outgassing_Gauss = gaussmf([-self.Model.Conditions(Run_Index).Constant.Outgassing_Spread*4:self.Model.Conditions(Run_Index).Constant.Outgassing_Temporal_Resolution:self.Model.Conditions(Run_Index).Constant.Outgassing_Spread*4],[self.Model.Conditions(Run_Index).Constant.Outgassing_Spread,0]);
        ...
        function RunModel(self,src,event);
            ...
            if self.ValidatedFlag;
                ...
                self.UpdateLogBox(['Starting...',' @ ',char(datetime('now','Format','HH:mm:ss'))]);
...
%                 try
                ...
                for Run = 1:RunNumber;
                    ...
                    for Chunk = 1:numel(self.Runs(Run).Chunks);
                        ...
                        % Run the solver
                        [DataChunks{Chunk},ConstChunks{Chunk},LysChunks{Chunk},CoreChunks{Chunk},SeafloorChunks{Chunk}] = self.Model.SolverFcn(ODEFunc,SingleRunModel,Time_In,Time_Out,SingleRunModel.Conditions.Initial.Conditions,SingleRunModel.Conditions.Initial.Seafloor,SingleRunModel.Conditions.Initial.Outgassing);
